...
final class ExtendedPrivateKeyR1Deserializer implements Deserializer<ExtendedPrivateKeyR1> {

    static final ExtendedPrivateKeyR1Deserializer DEFAULT = new ExtendedPrivateKeyR1Deserializer(DefaultNetworks.INSTANCE);

    private final Networks networks;

    ExtendedPrivateKeyR1Deserializer(final Networks networks) {
        this.networks = networks;
    }

    @Override
    public ExtendedPrivateKeyR1 deserialize(final CharSequence extendedBase58Key) {
    ...
    @Override
    public ExtendedPrivateKeyR1 deserialize(final byte[] extendedKeyData) {
        confirmExtendedKeyChecksum(extendedKeyData);
        final ByteArrayReader reader = new ByteArrayReader(extendedKeyData);
        return new ExtendedPrivateKeyR1(new HdKeyR1
                .Builder()
