 ...
 * A BIP32 private key
 */
public final class ExtendedPrivateKeyR1 implements
        Derive<ExtendedPrivateKeyR1>,
        ExtendedKey<ExtendedPrivateKeyR1>,
        CKDpriv<ExtendedPrivateKeyR1>,
        CKDpub<ExtendedPublicKeyR1>{

    public static Deserializer<ExtendedPrivateKeyR1> deserializer() {
        return ExtendedPrivateKeyR1Deserializer.DEFAULT;
    }

    public static Deserializer<ExtendedPrivateKeyR1> deserializer(final Networks networks) {
        return new ExtendedPrivateKeyR1Deserializer(networks);
    }

    private static final CkdFunction<ExtendedPrivateKeyR1> CKD_FUNCTION = new CkdFunction<ExtendedPrivateKeyR1>() {
        @Override
        public ExtendedPrivateKeyR1 deriveChildKey(final ExtendedPrivateKeyR1 parent, final int childIndex) {
    ...
    private ExtendedPrivateKeyR1(final Network network, final byte[] key, final byte[] chainCode) {
        this(new HdKeyR1.Builder()
//                .network(network)
//                .neutered(false)
                .key(key)
                .chainCode(chainCode)
...
//                .parentFingerprint(0)
    ...
    ExtendedPrivateKeyR1(final HdKeyR1 hdKey) {
        this.hdKey = hdKey;
    }

    public static ExtendedPrivateKeyR1 fromSeed(final byte[] seed, final Network network) {
        final byte[] I = hmacSha512(BITCOIN_SEED, seed);
        final byte[] Il = head32(I);
        final byte[] Ir = tail32(I);
        return new ExtendedPrivateKeyR1(network, Il, Ir);
    }
    ...
    @Override
    public ExtendedPrivateKeyR1 toNetwork(final Network otherNetwork) {
//        if (otherNetwork == network())
//            return this;
        return new ExtendedPrivateKeyR1(
                hdKey.toBuilder()
//                        .network(otherNetwork)
    ...
    public ExtendedPrivateKeyR1 cKDpriv(final int index ) {
        ...
        return new ExtendedPrivateKeyR1(new HdKeyR1.Builder()
...
//                .parentFingerprint(hdKey.calculateFingerPrint())
    ...
    public ExtendedPublicKeyR1 neuter() {
        return ExtendedPublicKeyR1.from(hdKey);
    }

    public Derive<ExtendedPrivateKeyR1> derive() {
        return derive(CKD_FUNCTION);
    }

    public Derive<ExtendedPrivateKeyR1> deriveWithCache() {
        return derive(newCacheOf(CKD_FUNCTION));
    }

    @Override
    public ExtendedPrivateKeyR1 derive(final CharSequence derivationPath) {
        return derive().derive(derivationPath);
    }

    @Override
    public <Path> ExtendedPrivateKeyR1 derive(final Path derivationPath, final Derivation<Path> derivation) {
        return derive().derive(derivationPath, derivation);
    }

    private Derive<ExtendedPrivateKeyR1> derive(final CkdFunction<ExtendedPrivateKeyR1> ckdFunction) {
