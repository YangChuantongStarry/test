...
final class ExtendedPublicKeyR1Deserializer implements Deserializer<ExtendedPublicKeyR1> {

    static final ExtendedPublicKeyR1Deserializer DEFAULT = new ExtendedPublicKeyR1Deserializer(DefaultNetworks.INSTANCE);

    private final Networks networks;

    ExtendedPublicKeyR1Deserializer(final Networks networks) {
        this.networks = networks;
    }

    @Override
    public ExtendedPublicKeyR1 deserialize(final CharSequence extendedBase58Key) {
    ...
    @Override
    public ExtendedPublicKeyR1 deserialize(final byte[] extendedKeyData) {
        confirmExtendedKeyChecksum(extendedKeyData);
        final ByteArrayReader reader = new ByteArrayReader(extendedKeyData);
        return new ExtendedPublicKeyR1(new HdKeyR1
                .Builder()
